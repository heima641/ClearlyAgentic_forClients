...
# -*- coding: utf-8 -*-
"""
Combined Case Study Processing Workflow Script - - - REVISED FOR CASE STUDY WORKFLOW

This script combines four sequential processes into a single workflow:

1. MODULE 1: Case Study Scraping
   - Retrieves a specified number of case studies from a chosen content category
   - Fetches their titles and text
   - Saves the results in a JSON file

2. MODULE 2: Content Chunking for Embeddings
   - Processes the retrieved case study content by chunking it into smaller pieces suitable for embeddings
   - Enriches the chunks with metadata
   - Saves the results in a second JSON file

3. MODULE 3: Advanced Chunk Processing
   - Further processes the chunked case study content with additional metadata
...
4. MODULE 4: Embedding Generation and Pinecone Upsert
   ...
   - Saves a complete record of all processed case study data
...
# =====================================================================
# MODULE 1: CASE STUDY SCRAPING IMPLEMENTATION
# =====================================================================

def extract_case_study_urls(sitemap_url, case_content_base_url, num_cases, case_sitemap_keyword, case_categ_options_sitemap_url=""): 
    """Extracts the most recent content URLs for a specified category from a sitemap."""
    if case_categ_options_sitemap_url:
        # Use the provided category-specific sitemap URL directly
        target_sitemap = case_categ_options_sitemap_url
    else:
        ...
        for link in sitemap_links:
            if case_sitemap_keyword in link:
                target_sitemap = link
                break
    ...
    if target_sitemap:
        ...
        # Filter to include only URLs that start with the CASE_CONTENT_BASE_URL and exclude images, PDFs, etc.
        content_links = [
            link for link in all_links
            if link.startswith(case_content_base_url) and not link.endswith(('.jpg', '.png', '.gif', '.pdf'))
        ]

    recent_links = content_links[-num_cases:]
...
def run_module_1_case_study_scraping(variables, case_json_path):
    """Module 1: Case Study Scraping
    Retrieves a specified number of case studies from a chosen content category,
    fetches their titles and text, and saves the results in a JSON file.
    
    Args:
        variables: Configuration variables from Supabase
        case_json_path: Path where the case study content will be saved
    
    Returns:
        str: Path to the generated JSON file
    """
    print("\n" + "=" * 80)
    print("MODULE 1: CASE STUDY SCRAPING")
    print("=" * 80)
    ...
    try:
        ...
        # Extract case study-specific configuration
        if "scripts" not in variables or "cases" not in variables["scripts"]:
            raise Exception("No 'scripts.cases' section found in Supabase config")
        ...
        # Get NUM_CASE
        if "NUM_CASE" not in case_config:
            raise Exception("NUM_CASE not found in Supabase config")
        num_cases = int(case_config["NUM_CASE"])
        print(f"Successfully fetched NUM_CASE from Supabase: {num_cases}")

        # Get CASE_SITEMAP_KEYWORD
        if "CASE_SITEMAP_KEYWORD" not in case_config:
            raise Exception("CASE_SITEMAP_KEYWORD not found in Supabase config")
        case_sitemap_keyword = case_config["CASE_SITEMAP_KEYWORD"]
        print(f"Successfully fetched CASE_SITEMAP_KEYWORD from Supabase: {case_sitemap_keyword}")

        # Get CASE_CATEG_OPTIONS_SITEMAP_URL (optional - can be empty string)
        if "CASE_CATEG_OPTIONS_SITEMAP_URL" in case_config:
            case_categ_options_sitemap_url = case_config["CASE_CATEG_OPTIONS_SITEMAP_URL"]
            print(f"Successfully fetched CASE_CATEG_OPTIONS_SITEMAP_URL from Supabase: {case_categ_options_sitemap_url}")
        else:
            case_categ_options_sitemap_url = ""
            print("CASE_CATEG_OPTIONS_SITEMAP_URL not found in Supabase, using empty string")

        # Get CASE_CONTENT_BASE_URL
        if "CASE_CONTENT_BASE_URL" not in case_config:
            raise Exception("CASE_CONTENT_BASE_URL not found in Supabase config")
        case_content_base_url = case_config["CASE_CONTENT_BASE_URL"]
        print(f"Successfully fetched CASE_CONTENT_BASE_URL from Supabase: {case_content_base_url}")
    ...
    # The output file path is provided by the caller (main workflow) via the case_json_path argument.
    ...
    print(f"CASE_CONTENT_BASE_URL: {case_content_base_url}")

    # --- Part 1: Generate a list of the case study URLs ---
    recent_items = extract_case_study_urls(sitemap_url, case_content_base_url, num_cases, case_sitemap_keyword, case_categ_options_sitemap_url)

    print(f"\n {company_name} - Latest {num_cases} case studies for category '{case_sitemap_keyword}':")
    ...
    # # SUPABASE BUCKET: upload raw case study JSON to Supabase bucket
    # SUP BUCKET OUTPUT SUP BUCKET OUTPUT
    with open(case_json_path, "rb") as _f:
        file_content = _f.read()
    response = supabase.storage.from_("agentic-output").upload(os.path.basename(case_json_path), 
        file_content,
        {"content-type": "application/json"}
    )
    print(f"Uploaded to Supabase: {os.path.basename(case_json_path)}, Response: {response}")

    print(f"Case study data saved to {case_json_path}")
...
# --- Chunking Function for Case Studies ---
def chunk_case_study_by_chars(text, min_chars=900, max_chars=1300):
    try:
        sentences = sent_tokenize(text)
    except Exception as e:
        log_punkt_tab_error("chunk_case_study")
...
def run_module_2_content_chunking(input_json_path, output_chunk_path):
    ...
    try:
        # Read the input case studies JSON file
        with open(input_json_path, 'r', encoding='utf8') as f:
            case_studies = json.load(f)

        enriched_posts = []
        print(f"Processing {len(case_studies)} case studies for chunking...")

        # Instantiate the analyzer
        analyzer = Analyzer()

        for idx, case in enumerate(case_studies):
            title = case.get('title', f"CaseStudy_{idx+1}")
            url = case.get('url', '')
            content = case.get('content', '')

            if not content:
                print(f"Skipping case study #{idx+1} ({title}) - No content found")
                continue

            # Split content into chunks based on character length and sentiment shifts
            chunks = chunk_case_study_by_chars(content, min_chars=900, max_chars=1300)
            print(f"Case study #{idx+1} ({title}) - Split into {len(chunks)} chunks")
            ...
            for i, chunk in enumerate(chunks):
                ...
                enriched_chunk = {
                    ...
                    "script_version": "CASE-STUDY-1.0"
                }
                enriched_chunk.update(metadata)
                enriched_posts.append(enriched_chunk)

        # Write the enriched case studies to the output JSON file
        ...
        # # SUPABASE BUCKET: upload chunked case study JSON to Supabase bucket
        ...
        print(f"Uploaded to Supabase: {os.path.basename(output_chunk_path)}, Response: {response}")

        print("Processing complete! All case studies have been processed and enriched.")
...
def run_module_4_embedding_generation(input_processed_path, output_embeddings_path):
    ...
    try:
        ...
        # Process Case Study Chunks
        print("Processing case study chunks and generating embeddings...")
        records = []
        ...
        for i, chunk in enumerate(chunks):
            try:
                ...
                sanitized_id = sanitize_id(chunk.get("id", f"case_study_chunk_{i}"))
                ...
                record = {
                    ...
                    "metadata": {
                        ...
                        "case_study_title": chunk.get("case_study_title", ""),
                        "case_study_url": chunk.get("case_study_url", ""),
                ...
                if (i + 1) % 10 == 0 or i == total_chunks - 1:
                    print(f"Processed {i + 1}/{total_chunks} case study chunks")

            except Exception as e:
                print(f"Error processing chunk {i}: {e}")
                # Continue with next chunk rather than failing entire process

        print(f"Successfully processed {len(records)}/{total_chunks} case study chunks")

        # Save Processed Records to a JSON File
        # SUP BUCKET OUTPUT SUP BUCKET OUTPUT
        print(f"Saving processed case study records to {output_embeddings_path}...")
        ...
        print(f"Uploaded to Supabase: {os.path.basename(output_embeddings_path)}, Response: {response}")
        print(f" Saved case study embeddings to {output_embeddings_path}")

        # Insert Embeddings into Pinecone
        print("Inserting case study embeddings into Pinecone...")
        batch_size = 100
        ...
        print(f"Completed Pinecone insertion. Inserted {successful_inserts}/{len(records)} case study vectors")

        # Verify Pinecone Storage - with detailed logging
        print("Verifying Pinecone storage for case study vectors...")
        ...
        print(f"Input case study chunks processed: {total_chunks}")
        print(f"Successfully embedded case study chunks: {len(records)}")
        print(f"Case study vectors added to Pinecone: {successful_inserts}")
        print(f"Initial vector count: {initial_vector_count}")
        print(f"Final vector count (Pinecone): {actual_total_vectors}")
        print(f"Case study vectors added in this run: {successful_inserts}")
...
def main():
    ...
    print("\n" + "=" * 80)
    print("COMBINED CASE STUDY PROCESSING WORKFLOW")
    print("=" * 80)
    ...
    try:
        ...
        case_sitemap_keyword = global_config.get("CASE_SITEMAP_KEYWORD", "case")
        num_cases = int(global_config.get("NUM_CASE", 10))
        ...
        # OUTPUT_DIR was removed from the script in favor of Supabase bucket - - SUP BUCKET OUTPUT
        case_json_path = f"{company_name_lower}_{actual_num_cases}_case_studies_{date_time}.json"
        chunk_json_path = f"{company_name_lower}_{actual_num_cases}_case_studies_chunked_{date_time}.json"
        embeddings_json_path = f"{company_name_lower}_{actual_num_cases}_case_studies_embeddings_{date_time}.json"
        ...
        print(f"Will process up to {num_cases} case studies with keyword '{case_sitemap_keyword}'")
        print(f"Output files will be saved to Supabase bucket: agentic-output")
        
        # Execute Module 1: Case Study Scraping
        case_json_path = run_module_1_case_study_scraping(variables, case_json_path)
        ...
        success = run_module_2_content_chunking(case_json_path, chunk_json_path)
        ...
        print(f"Case studies processed: {actual_num_cases}")
        print(f"Keyword: {case_sitemap_keyword}")
        print(f"Total execution time: {int(minutes)} minutes and {seconds:.2f} seconds")
        print(f"Output files:")
        print(f"  - Case study content: {os.path.basename(case_json_path)}")
