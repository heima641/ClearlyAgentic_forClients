...
class PoppyCardGenerator:
    ...
    def parse_standard_analysis_content(self, content: str) -> tuple[str, Dict[int, str]]:
        ...
        # Split content into sections - look for problem patterns
        # Handle different formats seen in the examples
        
        # Pattern 1: "### 1. Problem:" or "**1. Problem:"
        pattern1 = r'(?:###|##|\*\*)\s*(\d+)\.?\s*Problem[:\s]'
        
        # Pattern 2: "**Problem 1:" or "Problem 1:"
        pattern2 = r'\*?\*?Problem\s+(\d+):'
        ...
        if matches1:
            problem_matches = matches1
        else:
            # Try pattern 2
            matches2 = list(re.finditer(pattern2, content, re.IGNORECASE))
            problem_matches = matches2
        
        if not problem_matches:
            raise ValueError("Could not find problem statements in the expected format")
        ...
        for i, match in enumerate(problem_matches):
            ...
            # Find end position (start of next problem or end of file)
            if i + 1 < len(problem_matches):
                end_pos = problem_matches[i + 1].start()
            else:
                # For the last problem, find a reasonable end point
                # Look for ICP summary or end of file
                icp_match = re.search(r'(?:###|##|\*\*)\s*Ideal Customer Profile', content[start_pos:], re.IGNORECASE)
                if icp_match:
