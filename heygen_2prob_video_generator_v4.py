...
# -*- coding: utf-8 -*-
"""
HeyGen 2-Problem Video Generator v4 - Enhanced with Success Principles

This script processes chunked 2-problem video scripts and generates videos using HeyGen Template API:
- Reads chunked scripts from two-problem-script-drafts bucket
- Parses {{two_prob_slide_XX_content}} placeholders (10 slides) - FIXED NAMING
- Calls HeyGen Template API using proven success pattern from successful curl
...
- Placeholder naming: two_prob_slide_01_content (no leading numbers)
...
# 2-Problem Template Configuration (10 slides) - UPDATED FOR NEW NAMING
TEMPLATE_2PROB_CONFIG = {
    "template_id": "400edab00fdd4e45afb43430af749447",  # TWO SLIDES API 30JUL25
    "slide_count": 10,
...
logging.basicConfig(
    level=logging.INFO,
    format='%(asctime)s - [HEYGEN-2PROB-VIDEO] - %(levelname)s - %(message)s',
    handlers=[
        logging.StreamHandler(),
        logging.FileHandler(
            f"heygen_2prob_video_log_{datetime.now(eastern_tz).strftime('%Y%m%d_%H%M%S')}.log")
    ])
logger = logging.getLogger(__name__)

# Log initialization for HeyGen 2-problem video generation
logger.info("=" * 60)
logger.info("HEYGEN 2-PROBLEM VIDEO GENERATOR v4 - SESSION START")
logger.info(f"Target: Generate videos from chunked 2-problem scripts (10 slides)")
logger.info(f"Template ID: {TEMPLATE_2PROB_CONFIG['template_id']}")
logger.info(f"Processing bucket: {TEMPLATE_2PROB_CONFIG['bucket_name']}")
logger.info(f"Output bucket: {TEMPLATE_2PROB_CONFIG['output_bucket']}")
...
def fetch_configuration_from_supabase(config_name: Optional[str] = None, config_id: Optional[int] = None) -> Dict:
    ...
    """
    try:
        logger.info(f"[HEYGEN-2PROB-VIDEO-CONFIG] Fetching configuration from Supabase...")

        if config_id:
            logger.info(f"[HEYGEN-2PROB-VIDEO-CONFIG] Fetching configuration with ID: {config_id}")
            response = supabase.table("workflow_configs").select("*").eq("id", config_id).execute()
        elif config_name:
            logger.info(f"[HEYGEN-2PROB-VIDEO-CONFIG] Fetching configuration with name: {config_name}")
            response = supabase.table("workflow_configs").select("*").eq("config_name", config_name).execute()
        else:
            logger.info("[HEYGEN-2PROB-VIDEO-CONFIG] Fetching most recent configuration")
        ...
        logger.info(f"[HEYGEN-2PROB-VIDEO-CONFIG] Successfully fetched configuration: {config_name}")
        return config_data.get("variables", {})

    except Exception as e:
        logger.error(f"[HEYGEN-2PROB-VIDEO-CONFIG] Error fetching configuration from Supabase: {str(e)}")
        raise
...
def download_file_from_bucket(bucket_name: str, file_name: str) -> str:
    ...
    """
    try:
        logger.info(f"[HEYGEN-2PROB-VIDEO-DOWNLOAD] Downloading {file_name} from {bucket_name} bucket...")
        response = supabase.storage.from_(bucket_name).download(file_name)

        if response:
            content = response.decode('utf-8')
            logger.info(f"[HEYGEN-2PROB-VIDEO-DOWNLOAD] Successfully downloaded {file_name} ({len(content)} characters)")
            return content
        else:
            raise Exception(f"Failed to download {file_name}")

    except Exception as e:
        logger.error(f"[HEYGEN-2PROB-VIDEO-DOWNLOAD] Error downloading {file_name} from {bucket_name}: {str(e)}")
        raise
...
def upload_file_to_bucket(bucket_name: str, file_name: str, file_content: str) -> bool:
    ...
    """
    try:
        logger.info(f"[HEYGEN-2PROB-VIDEO-UPLOAD] Uploading {file_name} to {bucket_name} bucket...")
        ...
        logger.info(f"[HEYGEN-2PROB-VIDEO-UPLOAD] Successfully uploaded {file_name} to {bucket_name}")
        return True

    except Exception as e:
        logger.error(f"[HEYGEN-2PROB-VIDEO-UPLOAD] Error uploading {file_name} to {bucket_name}: {str(e)}")
        raise
...
def list_files_in_bucket(bucket_name: str) -> List[str]:
    ...
    """
    try:
        logger.info(f"[HEYGEN-2PROB-VIDEO-LIST] Listing files in {bucket_name} bucket...")
        response = supabase.storage.from_(bucket_name).list()

        if response:
            file_names = [file['name'] for file in response if file['name'].endswith('.txt')]
            logger.info(f"[HEYGEN-2PROB-VIDEO-LIST] Found {len(file_names)} .txt files in {bucket_name}")
            return file_names
        else:
            return []

    except Exception as e:
        logger.error(f"[HEYGEN-2PROB-VIDEO-LIST] Error listing files in {bucket_name}: {str(e)}")
        raise
...
def upload_video_to_bucket(video_url: str, output_filename: str, bucket_name: str = "heygen-generated-videos") -> Tuple[bool, str]:
    ...
    """
    try:
        logger.info(f"[HEYGEN-2PROB-VIDEO-UPLOAD] Downloading and uploading video: {output_filename}")
        ...
        if response.status_code == 200:
            ...
            logger.info(f"[HEYGEN-2PROB-VIDEO-UPLOAD] Downloaded video: {file_size_mb:.1f} MB")
            ...
            logger.info(f"[HEYGEN-2PROB-VIDEO-UPLOAD] Successfully uploaded video: {output_filename} ({file_size_mb:.1f} MB)")
            logger.info(f"[HEYGEN-2PROB-VIDEO-UPLOAD] Bucket URL: {bucket_url}")
            return True, bucket_url
        else:
            raise Exception(f"Failed to download video: {response.status_code}")
            
    except Exception as e:
        logger.error(f"[HEYGEN-2PROB-VIDEO-UPLOAD] Error uploading video: {str(e)}")
        return False, ""
...
def enhanced_rate_limit_handler(attempt: int, error_msg: str, retry_delay: int) -> int:
    ...
    if "rate" in error_lower and "limit" in error_lower:
        ...
        if "minute" in error_lower:
            ...
            logger.warning(f"[HEYGEN-2PROB-VIDEO-RATE-LIMIT] Per-minute rate limit - waiting {backoff_delay}s")
        elif "hour" in error_lower:
            # Hourly limit - much longer wait
            backoff_delay = 3600 + (attempt * 600)  # 1hr+, 1hr 10min+, etc.
            logger.warning(f"[HEYGEN-2PROB-VIDEO-RATE-LIMIT] Hourly rate limit - waiting {backoff_delay}s")
        else:
            # Generic rate limit
            backoff_delay = retry_delay * 3
            logger.warning(f"[HEYGEN-2PROB-VIDEO-RATE-LIMIT] Generic rate limit - waiting {backoff_delay}s")
        ...
        logger.warning(f"[HEYGEN-2PROB-VIDEO-QUOTA] Quota/billing issue - waiting {backoff_delay}s")
        return backoff_delay
    
    elif "timeout" in error_lower or "connection" in error_lower:
        # Network issues - moderate backoff
        backoff_delay = retry_delay * 2
        logger.warning(f"[HEYGEN-2PROB-VIDEO-NETWORK] Network issue - waiting {backoff_delay}s")
        return backoff_delay
...
def parse_chunked_script_placeholders(script_content: str) -> Dict[str, str]:
    ...
    """
    try:
        logger.info("[HEYGEN-2PROB-VIDEO-PARSE] Parsing chunked script placeholders...")
        
        # FIXED: Pattern to match new naming format: {{two_prob_slide_XX_content}}
        placeholder_pattern = r'\{\{(two_prob_slide_\d{2}_content)\}\}\s*\n(.+?)(?=\n\{\{two_prob_slide_\d{2}_content\}\}|\n#|\Z)'
        ...
        for placeholder_name, content in matches:
            ...
            if clean_content:
                ...
                logger.debug(f"[HEYGEN-2PROB-VIDEO-PARSE] {placeholder_name}: {len(clean_content)} chars")
        
        logger.info(f"[HEYGEN-2PROB-VIDEO-PARSE] Parsed {len(slide_content)} slide placeholders")
        
        # Validate we have expected slides (1-10)
        expected_slides = [f"two_prob_slide_{i:02d}_content" for i in range(1, 11)]
        missing_slides = [slide for slide in expected_slides if slide not in slide_content]
        if missing_slides:
            logger.warning(f"[HEYGEN-2PROB-VIDEO-PARSE] Missing slides: {missing_slides}")
        
        return slide_content
        
    except Exception as e:
        logger.error(f"[HEYGEN-2PROB-VIDEO-PARSE] Error parsing placeholders: {str(e)}")
        raise
...
def generate_video_with_retry(template_id: str, slide_content: Dict[str, str], video_title: str, 
    ...
    # SUCCESS PRINCIPLE: Build variables payload exactly like successful curl
    variables = {}
    ...
    logger.info(f"[HEYGEN-2PROB-VIDEO-GENERATE] Using template endpoint: {url}")
    logger.info(f"[HEYGEN-2PROB-VIDEO-GENERATE] Payload structure: caption={payload['caption']}, title='{payload['title']}', variables={len(variables)} slides")
    
    for attempt in range(max_retries):
        try:
            logger.info(f"[HEYGEN-2PROB-VIDEO-GENERATE] Attempt {attempt + 1}/{max_retries} - Generating video: {video_title}")
            
            response = requests.post(url, headers=headers, json=payload, timeout=60)
            
            logger.info(f"[HEYGEN-2PROB-VIDEO-GENERATE] Response status: {response.status_code}")
            
            if response.status_code == 200:
                data = response.json()
                logger.info(f"[HEYGEN-2PROB-VIDEO-GENERATE] Response data: {data}")
                
                if 'data' in data and 'video_id' in data['data']:
                    video_id = data['data']['video_id']
                    logger.info(f"[HEYGEN-2PROB-VIDEO-GENERATE] SUCCESS - Video ID: {video_id}")
                    return video_id
                else:
                ...
                logger.error(f"[HEYGEN-2PROB-VIDEO-GENERATE] API Error Response: {error_text}")
                raise Exception(f"HeyGen API request failed: {response.status_code} - {error_text}")
                
        except Exception as e:
            error_msg = str(e)
            logger.error(f"[HEYGEN-2PROB-VIDEO-ERROR] Attempt {attempt + 1} failed: {error_msg}")
            
            if attempt < max_retries - 1:
                # Use enhanced rate limit handler
                retry_delay = enhanced_rate_limit_handler(attempt, error_msg, retry_delay)
                logger.info(f"[HEYGEN-2PROB-VIDEO-RETRY] Retrying video generation in {retry_delay}s...")
                time.sleep(retry_delay)
            else:
                logger.error(f"[HEYGEN-2PROB-VIDEO-FAILURE] Failed to generate video after {max_retries} attempts")
                raise
...
def check_video_status_with_retry(video_id: str, max_retries: int = DEFAULT_MAX_RETRIES, 
    ...
    for attempt in range(max_retries):
        try:
            logger.debug(f"[HEYGEN-2PROB-VIDEO-STATUS] Checking status for video: {video_id}")
            ...
            if response.status_code == 200:
                ...
                if 'data' in data:
                    ...
                    logger.debug(f"[HEYGEN-2PROB-VIDEO-STATUS] Video {video_id} status: {status}")
                    return data['data']
                else:
            ...
            error_msg = str(e)
            logger.error(f"[HEYGEN-2PROB-VIDEO-STATUS-ERROR] Attempt {attempt + 1} failed: {error_msg}")
            
            if attempt < max_retries - 1:
                retry_delay = enhanced_rate_limit_handler(attempt, error_msg, retry_delay)
                logger.info(f"[HEYGEN-2PROB-VIDEO-STATUS-RETRY] Retrying status check in {retry_delay}s...")
                time.sleep(retry_delay)
            else:
                logger.error(f"[HEYGEN-2PROB-VIDEO-STATUS-FAILURE] Failed to check status after {max_retries} attempts")
                raise
...
def monitor_video_generation(video_id: str, max_wait_minutes: int = 30) -> Dict:
    ...
    """
    logger.info(f"[HEYGEN-2PROB-VIDEO-MONITOR] Monitoring video generation: {video_id}")
    ...
    while True:
        try:
            ...
            if status == 'completed':
                logger.info(f"[HEYGEN-2PROB-VIDEO-MONITOR] Video generation completed: {video_id}")
                return status_data
            elif status == 'failed':
                ...
                logger.info(f"[HEYGEN-2PROB-VIDEO-MONITOR] Video {video_id} still {status}... ({elapsed_minutes:.1f}min elapsed)")
                time.sleep(check_interval)
            else:
                logger.warning(f"[HEYGEN-2PROB-VIDEO-MONITOR] Unknown status '{status}' for video {video_id}")
                time.sleep(check_interval)
                
        except Exception as e:
            logger.error(f"[HEYGEN-2PROB-VIDEO-MONITOR] Error monitoring video {video_id}: {str(e)}")
            raise
...
def process_single_chunked_script(filename: str, template_config: Dict) -> Dict:
    """
    Process one chunked 2-problem script file to generate video with bucket upload
    ...
    """
    try:
        logger.info(f"[HEYGEN-2PROB-VIDEO-PROCESS] Processing {filename}")
        ...
        logger.info(f"[HEYGEN-2PROB-VIDEO-PROCESS] Found {len(slide_content)} slides to process")
        
        # Generate video title from filename
        base_name = filename.replace('.txt', '').replace('chunked_2prob_script_', '')
        video_title = f"2-Problem Video - {base_name}"
        ...
        output_filename = f"heygen_2prob_video_{base_name}_{timestamp}.mp4"
        ...
        logger.error(f"[HEYGEN-2PROB-VIDEO-PROCESS] Error processing {filename}: {str(e)}")
        return {
            "filename": filename,
            "error": str(e),
            "status": "failed"
...
def process_all_chunked_scripts(template_config: Dict, company_name: str) -> Dict:
    """
    Process all chunked 2-problem scripts in bucket
    ...
    """
    logger.info(f"[HEYGEN-2PROB-VIDEO-MAIN] Processing all chunked scripts for company: {company_name}")
    ...
    # Filter for chunked 2-problem scripts for this company
    chunked_files = [f for f in all_files 
                    if f.startswith(company_name) and 'chunked_2prob_script_' in f]
    
    logger.info(f"[HEYGEN-2PROB-VIDEO-MAIN] Found {len(chunked_files)} chunked script files for {company_name}")
    
    if not chunked_files:
        logger.warning(f"[HEYGEN-2PROB-VIDEO-MAIN] No chunked scripts found for {company_name}. Looking for files starting with '{company_name}' containing 'chunked_2prob_script_'")
        logger.info(f"[HEYGEN-2PROB-VIDEO-MAIN] Available files: {[f for f in all_files if f.startswith(company_name)]}")
    ...
    for filename in chunked_files:
        ...
        if result["status"] == "success":
            ...
            logger.info(f"[HEYGEN-2PROB-VIDEO-SUCCESS] Generated video: {result.get('output_filename', 'Unknown')}")
        else:
            results["failed"] += 1
            logger.error(f"[HEYGEN-2PROB-VIDEO-FAILED] Failed to generate video for: {filename}")
    
    logger.info(f"[HEYGEN-2PROB-VIDEO-MAIN] Processing complete: {results['successful']}/{results['total_processed']} videos generated successfully")
    return results
...
def test_single_video_generation(test_filename: str = None, template_id: str = None) -> Dict:
    ...
    """
    logger.info("[HEYGEN-2PROB-VIDEO-TEST] Running single video generation test...")
    
    try:
        # Use default template config for test
        template_config = TEMPLATE_2PROB_CONFIG.copy()
        
        # Override template ID if provided
        if template_id:
            template_config["template_id"] = template_id
            logger.info(f"[HEYGEN-2PROB-VIDEO-TEST] Using override template ID: {template_id}")
        
        # Template ID is configured and ready
        logger.info(f"[HEYGEN-2PROB-VIDEO-TEST] Using template: {template_config['template_id']}")
        
        logger.info(f"[HEYGEN-2PROB-VIDEO-TEST] Template ID: {template_config['template_id']}")
        
        if test_filename:
            # Test specific file
            logger.info(f"[HEYGEN-2PROB-VIDEO-TEST] Testing with specified file: {test_filename}")
            ...
            chunked_files = [f for f in all_files if 'chunked_2prob_script_' in f]
            
            if not chunked_files:
                raise Exception(f"No chunked scripts found in bucket {template_config['bucket_name']}")
            
            test_file = chunked_files[0]
            logger.info(f"[HEYGEN-2PROB-VIDEO-TEST] Testing with file: {test_file}")
            result = process_single_chunked_script(test_file, template_config)
        
        return result
        
    except Exception as e:
        logger.error(f"[HEYGEN-2PROB-VIDEO-TEST] Test failed: {str(e)}")
...
def main():
    """Main function to orchestrate the entire HeyGen 2-problem video generation workflow."""
    try:
        print("=" * 80)
        print("HEYGEN 2-PROBLEM VIDEO GENERATOR v4 - SUCCESS PRINCIPLES APPLIED")
        print("=" * 80)
        print(f"Start time: {start_time.strftime('%Y-%m-%d %H:%M:%S')}")
        print(f"Script directory: {SCRIPT_DIR}")
        print(f"Target: Generate videos from chunked 2-problem scripts")
        print(f"Template ID: {TEMPLATE_2PROB_CONFIG['template_id']}")
        print(f"Bucket: {TEMPLATE_2PROB_CONFIG['bucket_name']}")
        print(f"Output Bucket: {TEMPLATE_2PROB_CONFIG['output_bucket']}")
        print()
        ...
        print("✅ Fixed placeholder naming (two_prob_slide_XX_content)")
        ...
        logger.info(f"[HEYGEN-2PROB-VIDEO-MAIN] Using configured template: {TEMPLATE_2PROB_CONFIG['template_id']}")

        # Fetch configuration from Supabase
        logger.info("[HEYGEN-2PROB-VIDEO-MAIN] Fetching configuration from Supabase...")
        ...
        logger.info(f"[HEYGEN-2PROB-VIDEO-MAIN] Using template: {template_config['template_id']}")
        logger.info(f"[HEYGEN-2PROB-VIDEO-MAIN] Processing bucket: {template_config['bucket_name']}")
        logger.info(f"[HEYGEN-2PROB-VIDEO-MAIN] Output bucket: {template_config['output_bucket']}")
        logger.info(f"[HEYGEN-2PROB-VIDEO-MAIN] Company: {company_name}")
        ...
        print("\n" + "=" * 80)
        print("HEYGEN 2-PROBLEM VIDEO GENERATION SUMMARY")
        print("=" * 80)
        ...
        logger.info("=" * 60)
        logger.info("HEYGEN 2-PROBLEM VIDEO GENERATOR v4 - SESSION END (SUCCESS)")
        logger.info("=" * 60)

    except Exception as e:
        logger.critical(f"[HEYGEN-2PROB-VIDEO-CRITICAL] Critical error in video generation workflow: {str(e)}")
        logger.critical(f"[HEYGEN-2PROB-VIDEO-CRITICAL] Traceback: {traceback.format_exc()}")
        print(f"\n❌ Critical error in video generation workflow: {str(e)}")
        print(f"Traceback: {traceback.format_exc()}")
        
        # Log session end on failure
        logger.info("=" * 60)
        logger.info("HEYGEN 2-PROBLEM VIDEO GENERATOR v4 - SESSION END (FAILED)")
        logger.info("=" * 60)
        raise
