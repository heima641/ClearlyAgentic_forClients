...
All output files are saved to a Supabase Bucket.
"""
...
def run_module_1_blog_scraping(variables, blog_json_path):
    ...
    response = supabase.storage.from_("agentic-output").upload(
        os.path.basename(blog_json_path), 
        file_content, 
        {"content-type": "application/json"}
    )
    print(f"Uploaded to Supabase: {os.path.basename(blog_json_path)}, Response: {response}")
...
def main():
    ...
    try:
        ...
        blog_json_path = f"{company_name_lower}_{actual_num_blogs}_blogs_{date_time}.json"
        chunk_json_path = f"{company_name_lower}_{actual_num_blogs}_blogs_chunked_{date_time}.json"
        embeddings_json_path = f"{company_name_lower}_{actual_num_blogs}_blogs_embeddings_{date_time}.json"
