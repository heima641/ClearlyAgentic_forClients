...
# -*- coding: utf-8 -*-
"""
Combined Press Release Processing Workflow Script

This script combines four sequential processes into a single workflow focused on press releases:

1. MODULE 1: Press Release Scraping
...
2. MODULE 2: Content Chunking for Embeddings
   - Processes the retrieved press release content by chunking it into smaller pieces suitable for embeddings
   - Enriches the chunks with metadata
   - Saves the results in a second JSON file

3. MODULE 3: Advanced Chunk Processing
   - Further processes the chunked press release content with additional metadata
...
# Helper function to avoid repetitive error messages -- See 94-109 and 94-117 regarding Punkt
...
logging.basicConfig(
    ...
    handlers=[
        ...
        logging.FileHandler(f"press_release_embedding_log_{datetime.now().strftime('%Y%m%d_%H%M%S')}.log")
    ]
)
...
# =====================================================================
# MODULE 1: PRESS_RELEASE SCRAPING IMPLEMENTATION
# =====================================================================

def extract_content_urls(sitemap_url, press_content_base_url, num_press_releases, press_sitemap_keyword, press_categ_options_sitemap_url=""):
    """Extracts the most recent content URLs for a specified category from a sitemap."""
    if press_categ_options_sitemap_url:
        # Use the provided category-specific sitemap URL directly
        target_sitemap = press_categ_options_sitemap_url
    else:
        ...
        for link in sitemap_links:
            if press_sitemap_keyword in link:
                target_sitemap = link
                break
    ...
    if target_sitemap:
        ...
        # Filter to include only URLs that start with the PRESS_CONTENT_BASE_URL and exclude images, PDFs, etc.
        content_links = [
            link for link in all_links
            if link.startswith(press_content_base_url) and not link.endswith(('.jpg', '.png', '.gif', '.pdf'))
        ]

    recent_links = content_links[-num_press_releases:]
...
def run_module_1_press_release_scraping(variables, press_release_json_path):
    ...
    """
    print("\n" + "=" * 80)
    print("MODULE 1: PRESS_RELEASE SCRAPING")
    print("=" * 80)
    ...
    try:
        ...
        # Extract press_release-specific configuration
        if "scripts" not in variables or "press" not in variables["scripts"]:
            raise Exception("No 'scripts.press' section found in Supabase config")
        
        press_config = variables["scripts"]["press"]
        
        # Get COMPANY_NAME from global section (since it's not in press section)
        ...
        # Get SITEMAP_URL from global section (since it's not in press_releases section)
        ...
        # Get NUM_PRESS
        if "NUM_PRESS" not in press_config:
            raise Exception("NUM_PRESS not found in Supabase config")
        num_press_releases = int(press_config["NUM_PRESS"])
        print(f"Successfully fetched NUM_PRESS from Supabase: {num_press_releases}")

        # Get PRESS_SITEMAP_KEYWORD
        if "PRESS_SITEMAP_KEYWORD" not in press_config:
            raise Exception("PRESS_SITEMAP_KEYWORD not found in Supabase config")
        press_sitemap_keyword = press_config["PRESS_SITEMAP_KEYWORD"]
        print(f"Successfully fetched PRESS_SITEMAP_KEYWORD from Supabase: {press_sitemap_keyword}")

        # Get PRESS_CATEG_OPTIONS_SITEMAP_URL (optional - can be empty string)
        if "PRESS_CATEG_OPTIONS_SITEMAP_URL" in press_config:
            press_categ_options_sitemap_url = press_config["PRESS_CATEG_OPTIONS_SITEMAP_URL"]
            print(f"Successfully fetched PRESS_CATEG_OPTIONS_SITEMAP_URL from Supabase: {press_categ_options_sitemap_url}")
        else:
            press_categ_options_sitemap_url = ""
            print("PRESS_CATEG_OPTIONS_SITEMAP_URL not found in Supabase, using empty string")

        # Get PRESS_CONTENT_BASE_URL
        if "PRESS_CONTENT_BASE_URL" not in press_config:
            raise Exception("PRESS_CONTENT_BASE_URL not found in Supabase config")
        press_content_base_url = press_config["PRESS_CONTENT_BASE_URL"]
        print(f"Successfully fetched PRESS_CONTENT_BASE_URL from Supabase: {press_content_base_url}")
    ...
    print(f"PRESS_CONTENT_BASE_URL: {press_content_base_url}")

    # --- Part 1: Generate a list of the content URLs ---
    recent_items = extract_content_urls(sitemap_url, press_content_base_url, num_press_releases, press_sitemap_keyword, press_categ_options_sitemap_url)

    print(f"\n🔹 {company_name} - Latest {num_press_releases} items for category '{press_sitemap_keyword}':")
    ...
    # --- Part 3: Save the scraped data to a JSON file (to Supabase bucket) ---
    # First write to a temporary local file
    with open(press_release_json_path, "w") as f:
    ...
    return press_release_json_path
...
def chunk_press_release_post_by_chars(text, min_chars=900, max_chars=1300):
    try:
        ...
        log_punkt_tab_error("chunk_press_release_post")
...
def run_module_2_content_chunking(input_json_path, output_chunk_path):
    ...
    try:
        # Read the input press_release posts JSON file
        with open(input_json_path, 'r', encoding='utf8') as f:
            posts = json.load(f)

        enriched_posts = []
        print(f"Processing {len(posts)} press_release posts for chunking...")
        ...
        for idx, post in enumerate(posts):
            ...
            if not content:
                print(f"Skipping press_release post #{idx+1} ({title}) - No content found")
                continue
            ...
            for i, chunk in enumerate(chunks):
                ...
                enriched_chunk = {
                    ...
                    "script_version": "PRESS_RELEASE-1.0"
                }
                enriched_chunk.update(metadata)
                enriched_posts.append(enriched_chunk)

        # Write the enriched press_release posts to the output JSON file and upload to Supabase
        # First write to a temporary local file
    ...
    try:
        ...
        for i, chunk in enumerate(chunks):
            try:
                ...
                sanitized_id = sanitize_id(chunk.get("id", f"press_release_chunk_{i}"))
                ...
                record = {
                    ...
                    "metadata": {
                        ...
                        "press_release_title": chunk.get("press_release_title", ""),
                        "press_release_url": chunk.get("press_release_url", ""),
...
def main():
    ...
    print("\n" + "=" * 80)
    print("COMBINED PRESS_RELEASE PROCESSING WORKFLOW")
    print("=" * 80)
    ...
    try:
        ...
        press_sitemap_keyword = global_config.get("PRESS_SITEMAP_KEYWORD", "press_release")
        num_press_releases = int(global_config.get("NUM_PRESS", 10))
        
        # No longer need to create local output directory as files will be saved to Supabase bucket
        ...
        # Get the actual number of press_releases from the press_config
        press_config = variables["scripts"]["press"]
        actual_num_press = int(press_config.get("NUM_PRESS", num_press_releases))
        
        # Define file paths without OUTPUT_DIR as they'll be uploaded to Supabase bucket
        press_release_json_path = f"{company_name_lower}_{actual_num_press}_press_releases_{date_time}.json"
        chunk_json_path = f"{company_name_lower}_{actual_num_press}_press_releases_chunked_{date_time}.json"
        embeddings_json_path = f"{company_name_lower}_{actual_num_press}_press_releases_embeddings_{date_time}.json"
        ...
        success = run_module_2_content_chunking(press_release_json_path, chunk_json_path)
